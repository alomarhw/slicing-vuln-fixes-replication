...
def iso_lift(ax, xmax=0.8):
    ...
    Each is labeled just below where it leaves the plot, clear of the data labels."""
    ...
    for lift, lab in ((1.0, "lift 1.0× (random)"), (1.5, "1.5×"), (2.0, "2.0×")):
        ...
        if lift == 1.0:   # the random line crosses empty space at the lower left
            ...
            xe = xs[m][-1] - 0.006
            ax.text(xe, lift * xe - 0.035, lab, color=INK2, fontsize=6.2, ha="right", va="top")
...
def region_map():
    ...
    rsr = full["mean_rsr"]
    a_pts = [
        ("Backward slice (all sinks)", rsr, full["mean_coverage"], BLUE, "o", 0.014, 0.0, "left"),
        ("Memory sinks", sinks["memory"]["mean_rsr"], sinks["memory"]["mean_coverage"], BLUE, "o", 0.014, 0.0, "left"),
        ("API-call sinks", sinks["api"]["mean_rsr"], sinks["api"]["mean_coverage"], BLUE, "o", 0.014, 0.0, "left"),
        ("Variable mention", b["variable_mention"]["mean_rsr"], b["variable_mention"]["mean_coverage_deletion"], AQUA, "s", -0.016, 0.0, "right"),
        ("Sink window", b["sink_window"]["mean_rsr"], b["sink_window"]["mean_coverage_deletion"], AQUA, "s", 0.014, 0.0, "left"),
        ("Random window", rsr, b["random_window"]["mean_coverage_deletion"], AQUA, "s", 0.014, 0.012, "left"),
        ("Tail", rsr, b["tail"]["mean_coverage_deletion"], AQUA, "s", -0.014, 0.0, "right"),
        ("Random lines", rsr, b["random_lines"]["mean_coverage_deletion"], AQUA, "s", 0.014, -0.016, "left"),
    ]
    b_pts = [
        ("Backward from sinks", d["backward"]["mean_rsr"], d["backward"]["mean_coverage"], BLUE, "o", -0.016, 0.025, "right"),
        ("Backward, data only", d["backward_data"]["mean_rsr"], d["backward_data"]["mean_coverage"], BLUE, "o", 0.014, -0.02, "left"),
        ("Union", d["union"]["mean_rsr"], d["union"]["mean_coverage"], ORANGE, "^", 0.014, 0.0, "left"),
        ("Forward from inputs", d["forward"]["mean_rsr"], d["forward"]["mean_coverage"], ORANGE, "^", 0.014, 0.0, "left"),
